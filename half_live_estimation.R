...
# dat2=read.csv('../../Tafenoquine_PK_datsets/Results TQ James.csv')
# 
...
# outcome_dat$t_12 = -log(2)/outcome_dat$slope
# x_preds=posterior_predict(mod_cens)

# pdf('comparison_half_lives.pdf',width = 8, height = 8)
...
#                                 ((outcome_dat$t_12-outcome_dat$t_12_NONMEM)>3.7 |
#                                    (outcome_dat$t_12-outcome_dat$t_12_NONMEM) < -4) ]
# par(mfrow=c(4,4), mar=c(2,5,3,1),las=1)
...
#   lines(pk$TIME[ind], colMeans(x_preds[,ind]),lwd=3, lty=2)
#   polygon(c(pk$TIME[ind], rev(pk$TIME[ind])), 
#           c(apply(x_preds[,ind],2,quantile,prob=0.1),
#             rev(apply(x_preds[,ind],2,quantile,prob=0.9))),border = NA,
...
# 
# library(lme4)
# summary(glmer(outcome_primary ~ t_12 + tqmgkgtot + (1|studysite), family='binomial', data = outcome_dat))

# summary(glmer(outcome_primary ~ t_12_NONMEM + tqmgkgtot + (1|studysite), 
#               family='binomial', data = outcome_dat))
# 
# summary(glmer(outcome_primary ~ t_12_NONMEM + tqmgkgtot + (1|studysite), 
#               family='binomial', data = outcome_dat[!outcome_dat$pid %in% ids_outlier,]))
