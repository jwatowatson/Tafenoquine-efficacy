...
plot(survmod, col = mycols, lty=1:5,
...
grid()
legend('bottomleft', col=mycols, lwd=c(2,2,2,2,2,4), lty=1:5,
       legend = c('No Tafenoquine (n=186)',
                  'Tafenoquine: <3.75 mg/kg (n=170)',
                  'Tafenoquine: 3.75-6.25 mg/kg (n=379)',
                  'Tafenoquine: 6.25-8.75 mg/kg (n=56)',
                  'Tafenoquine: >8.75 mg/kg (n=44)',
                  'Primaquine: 1.76-6.0 mg/kg (n=265)'), inset=0.03)
abline(v=c(120, 180), lty=2,lwd=2)
...
legend('topleft', lty=1:2, lwd=2, legend = c('All (4 mths)', 'All (6 mths)'))
...
par(las=1, cex.lab=1.5, cex.axis=1.5,mar=c(5,5,0,0),family='serif')

plot(pred_x, 100*ps2, lwd=3, type='l',
     xlab = '',col=mycols[1], ylab = '',
     ylim = c(0,65), panel.first=grid())
mtext(text = substitute(paste(italic("P. vivax "), 'recurrence (%)')), 
      side = 2,line = 3,las=3,cex=1.8)
mtext(text = 'Single dose tafenoquine (mg/kg)', 
      side = 1,line = 3,las=1,cex=1.8)
abline(h = 100*invlogit(mean(thetas_all$E0 + 
                               3.5*thetas_all$beta[,1] + 
                               para_density*thetas_all$beta[,2])),
       lty=1, lwd=2,col='darkred')
