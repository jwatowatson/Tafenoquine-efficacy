...
pk_summaries$t_12_terminal[!is.na(pk_summaries$t_12_terminal) & pk_summaries$t_12_terminal>31]=NA

mod_day7_3 = lm(log10(day7_mthb) ~ tqmgkgtot + t_12_terminal,data=pk_summaries)
summary(mod_day7_3)

mod_day7_3 = MASS::rlm(log10(day7_mthb) ~ tqmgkgtot + t_12_terminal,
                       data=pk_summaries)
plot(pk_summaries$t_12_terminal, log10(pk_summaries$day7_mthb),
...
lines(5:30, predict(mod_day7_3, data.frame(tqmgkgtot=5, t_12_terminal=5:30)),lwd=3,lty=2)
```
...
```{r}
mod_t_12 = glmer(outcome7to180 ~ t_12_terminal + tqmgkgtot + logpara0 + (1|studysite),
                 family='binomial', 
                 data = outcome_dat)
...
writeLines(sprintf('Using all data (n=%s), the odds ratio for recurrence at 6 months for each additional day in the tafenoquine half life is %s (95%% CI %s to %s)',
                   ...
                   round(exp(fixef(mod_t_12)['t_12_terminal']),2),
                   round(exp(mod_t_12_out$coefficients['t_12_terminal',1:2]%*%c(1,-1.96)),2),
                   round(exp(mod_t_12_out$coefficients['t_12_terminal',1:2]%*%c(1,1.96)),2)))
