...
ggsurvplot(survmod, 
           ...
           censor = TRUE, xlim=c(0,180), ylim=c(0,1), 
           ...
           legend.title='', ylab='Recurrence free', 
           surv.scale='percent', 
...
# grid()
# legend('bottomleft', col=mycols, lwd=c(4,2,2,2,2,4), lty=1:5,
#        legend = c(paste0('No Tafenoquine (n=',xx$n[1],')'),
#                   paste0('Tafenoquine: <3.75 mg/kg (n=',xx$n[2],')'),
#                   paste0('Tafenoquine: 3.75-6.25 mg/kg (n=',xx$n[3],')'),
#                   paste0('Tafenoquine: 6.25-8.75 mg/kg (n=',xx$n[4],')'),
#                   paste0('Tafenoquine: >8.75 mg/kg (n=',xx$n[5],')'),
#                   paste0('Primaquine: 1.76-6.0 mg/kg (n=',xx$n[6],')')),
#        inset=0.03)
...
hist(outcome_dat$weight,
     ylab='',xaxt='n',xlab='',yaxt='n',col = brewer.pal(8,'Pastel2')[7],
     main='', breaks = seq(30,140,by=5))
par(las=1, cex.lab=1.8, cex.axis=1.8,mar=c(6,6,0,2))
plot(30:140, 100*(f300(30:140)-f450(30:140)), type='l', lwd=3,
     ylim = c(0, 15), panel.first=grid(),
     xlab='Weight (kg)', ylab='Absolute reduction in recurrence (%)')
...
mod_Cmax_weightadj = stan_glmer(outcome_primary ~ log10(CMAX) + tqmgkgtot + logpara0 + (1|studysite),
                                family='binomial', data = outcome_dat,
                                verbose=FALSE,refresh=0)
writeLines(
  sprintf('Using all data (n=%s), and adjusted for TQ mg/kg dose, the odds ratio for recurrence at 6 months for each SD in TQ Cmax %s (95%% CI %s to %s)',
          nobs(mod_Cmax_weightadj),
          round(exp(fixef(mod_Cmax_weightadj)['log10(CMAX)']),2),
          round(exp(posterior_interval(mod_Cmax_weightadj,
                                       pars='log10(CMAX)',prob = 0.95)[1]),2),
          round(exp(posterior_interval(mod_Cmax_weightadj,
                                       pars='log10(CMAX)',prob = 0.95)[2]),2)))

plot(mod_Cmax_weightadj, pars=c('tqmgkgtot','log10(CMAX)'))
