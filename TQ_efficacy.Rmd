...
```{r add_Pk}
## PK summaries statistics from NONMEM model
pk_summaries = read.csv('TQ_PK_summaries.csv')
load('Rout/t12.Rdata') ## Robust t_12 estimates from log-linear model
colnames(t12_summary) = c('PID', 't_12')
pk_summaries = merge(pk_summaries, t12_summary,by='PID', all = T)


par(las=1, cex.axis=1.3, cex.lab=1.3)
plot(pk_summaries$t_12_terminal, pk_summaries$t_12,xlab='NONMEM',
     ylab='Robust log-linear', panel.first=grid())
lines(0:50, 0:50)
qqplot(pk_summaries$t_12_terminal, pk_summaries$t_12, panel.first=grid())

hist(pk_summaries$t_12, 
     xlab='Terminal elimination half life (days)',
     breaks = 20,main='', yaxt='n',ylab='')
writeLines(sprintf('The median half life is %s days',round(median(pk_summaries$t_12, na.rm = T),1)))

plot(log(pk_summaries$BW), pk_summaries$t_12)
# pk_summaries$t_12[pk_summaries$t_12>27]=NA
mod_t12_bw = lm(t_12 ~ log(BW), data = pk_summaries)
pk_summaries$t_12_bw_adjusted = NA
pk_summaries$t_12_bw_adjusted[!is.na(pk_summaries$t_12)] = mean(pk_summaries$t_12, na.rm=T) + residuals(mod_t12_bw)

cor.test(pk_summaries$t_12_bw_adjusted, pk_summaries$BW)

outcome_dat = merge(outcome_dat, pk_summaries[,c('PID','ACT','AUC','CMAX','t_12_bw_adjusted','t_12')], 
...
```{r pk_data_plot}
pk_database = readr::read_csv('../../Tafenoquine_PK_datsets/All_TQ_PK_data.csv',
                              na = '.')[-1, ]
...
pk_database = pk_database[pk_database$CMT==2, ] # get just observed concs
...
writeLines(sprintf('there are a total of %s concentrations above LOQ in %s patients',sum(pk_database$BLQ==0), length(unique(pk_database$PID))))
writeLines(sprintf('there are a total of %s concentrationsBLQ in %s patients',sum(pk_database$BLQ==1), length(unique(pk_database$PID))))
...
pk_database$LNDV[pk_database$BLQ==1]=log(1)
...
summary(lm(t_12_bw_adjusted ~ AS_score<=0.5, data = outcome_dat))
...
par(las=1, family='serif', cex.lab=1.3, cex.axis=1.3, mfrow=c(3,3))
for(cc in unique(outcome_dat$country)){
  hist(outcome_dat$tqmgkgtot[outcome_dat$country==cc & outcome_dat$tqmgkgtot>0],
       main=cc,xlab='TQ (mg/kg)',
       breaks = seq(0, 15, by=1), ylab='Number of patients')
}
```
...
for(i in 1:nrow(mthb_mat)){
  ...
  f_mthb = approxfun(x = mthb_dat$time_since_dose[ind], 
                     y = mthb_dat$hgbmet[ind],ties = 'mean')
...
ids = unique(outcome_dat$pid[outcome_dat$tqmgkgtot>=3.75 &
                               outcome_dat$tqmgkgtot<6.25])
...
# Above 6.25 mg/kg
ids = unique(outcome_dat$pid[outcome_dat$tqmgkgtot>=6.25])
plot(0:29, colMeans(mthb_mat[rownames(mthb_mat) %in% ids, ],na.rm = T),
     type='l',lwd=3, ylim = c(0, 6.5), ylab='MetHb (%)',
...
ind = outcome_dat$pqmgkgtot==0
plot(outcome_dat$tqmgkgtot[ind], log10(outcome_dat$day7_mthb[ind]), yaxt='n',
     ylab='Day 7 MetHb (%)', xlab='Tafenoquine dose (mg/kg)',
     ylim = log10(c(0.3, 10)),col=outcome_dat$outcome_primary[ind]+1,
     panel.first=grid(),
     pch = outcome_dat$outcome_primary[ind]*15+1)
...
# linear fit
mod_day7 = lm(log10(day7_mthb) ~ tqmgkgtot, data=outcome_dat[ind,])
lines(0:15, predict(mod_day7, data.frame(tqmgkgtot=0:15)),lty=2,lwd=3)
summary(mod_day7)

mod_day7_3 = lm(log10(day7_mthb) ~ tqmgkgtot+t_12_bw_adjusted,data=outcome_dat)
summary(mod_day7_3)
plot(outcome_dat$t_12_bw_adjusted, log10(outcome_dat$day7_mthb),
     xlab=expression(paste(t[1/2], ' (days)')), yaxt='n',
     ylab = 'Day 7 MetHb (%)', ylim = log10(c(0.3, 10)),
     panel.first=grid(), col=outcome_dat$outcome_primary+1,
     pch = outcome_dat$outcome_primary*15+1)
axis(2, at = 0:1, labels = 10^(0:1))
axis(2, at = log10(c(0.3, 3)), labels = c(0.3, 3))
axis(2, at = log10(seq(0.3, 1, by=.1)), labels = NA)
axis(2, at = log10(seq(1, 10, by=1)), labels = NA)
...
lines(10:25, predict(mod_day7_3, 
                     data.frame(tqmgkgtot=5, t_12_bw_adjusted=10:25)),
      lwd=3,lty=2)
```
...
writeLines(sprintf('Using all data (n=%s), the odds ratio for recurrence at 4 months for each additional 1 mg/kg of tafenoquine (20%% increase on the 5 mg/kg adult dose) is %s (95%% CI %s to %s)',
                   nobs(mod_mgkg_all),
...
writeLines(sprintf('Using all data (n=%s), the odds ratio for recurrence at 4 months for each additional 0.7 mg/kg of primaquine (20%% increase on the 3.5 mg/kg adult dose) is %s (95%% CI %s to %s)',
                   nobs(mod_mgkg_all),
                   round(exp(fixef(mod_mgkg_all)['pqmgkgtot']*0.7),2),
                   round(exp(posterior_interval(mod_mgkg_all,pars='pqmgkgtot',prob = 0.95)[1]*0.7),2),
                   round(exp(posterior_interval(mod_mgkg_all,pars='pqmgkgtot',prob = 0.95)[2]*0.7),2)))

preds = rstanarm::posterior_linpred(mod_mgkg_all,
                                    newdata = data.frame(pqmgkgtot=c(0,3.5), 
                                                         tqmgkgtot=c(5,0),
                                                         logpara0 =4, studysite= -1),
                                    re.form = ~0, transform=F)
mean(preds[,1]<preds[,2])
...
writeLines(
  sprintf('Using all data (n=%s), and adjusted for TQ mg/kg dose, the odds ratio for recurrence at 6 months for each SD in TQ AUC %s (95%% CI %s to %s)',
          ...
          round(exp(posterior_interval(mod_auc_weightadj,
                                       pars='scale(AUC)',prob = 0.95)[2]),2)))
plot(mod_auc_weightadj, pars=c('tqmgkgtot','scale(AUC)'))
...
```{r t_terminal}
mod=lm(t_12_bw_adjusted ~ country, data = outcome_dat)
summary(mod)
boxplot(t_12_bw_adjusted ~ country, data = outcome_dat,las=2,xlab='')


mod_t_12 = stan_glmer(outcome_primary ~ t_12_bw_adjusted + logpara0 + (1|studysite),
                      family='binomial', data = outcome_dat,
                      verbose=F, refresh=0)
writeLines(sprintf('Using all data (n=%s), the odds ratio for recurrence at 4 months for each additional day in the tafenoquine half life is %s (95%% CI %s to %s)',
                   nobs(mod_t_12),
                   round(exp(fixef(mod_t_12)['t_12_bw_adjusted']),2),
                   round(exp(posterior_interval(mod_t_12,pars='t_12_bw_adjusted',prob = 0.95)[1]),2),
                   round(exp(posterior_interval(mod_t_12,pars='t_12_bw_adjusted',prob = 0.95)[2]),2)))

mod_t_12_weightadj = stan_glmer(outcome_primary ~ t_12_bw_adjusted + tqmgkgtot + (1|studysite),
                                family='binomial', data = outcome_dat,
                                verbose=F, refresh=0)
writeLines(sprintf('Using all data (n=%s), with adjustment for TQ mg/kg dose, the odds ratio for recurrence at 4 months for each additional day in the tafenoquine half life is %s (95%% CI %s to %s)',
                   nobs(mod_t_12_weightadj),
                   round(exp(fixef(mod_t_12_weightadj)['t_12_bw_adjusted']),2),
                   round(exp(posterior_interval(mod_t_12_weightadj,pars='t_12_bw_adjusted',prob = 0.95)[1]),2),
                   round(exp(posterior_interval(mod_t_12_weightadj,pars='t_12_bw_adjusted',prob = 0.95)[2]),2)))

plot(mod_t_12_weightadj,pars=c('tqmgkgtot', 't_12_bw_adjusted'))

mod_t_12_sens = stan_glmer(outcome_primary ~ t_12_bw_adjusted + (1|studysite),
                           family='binomial', data = outcome_dat[ind_sensitivity,],
                           verbose=F, refresh=0)
writeLines(sprintf('Using patients given 300 mg (n=%s), the odds ratio for recurrence at 4 months for each additional day in the tafenoquine half life is %s (95%% CI %s to %s)',
                   nobs(mod_t_12_sens),
                   round(exp(fixef(mod_t_12_sens)['t_12_bw_adjusted']),2),
                   round(exp(posterior_interval(mod_t_12_sens,pars='t_12_bw_adjusted',prob = 0.95)[1]),2),
                   round(exp(posterior_interval(mod_t_12_sens,pars='t_12_bw_adjusted',prob = 0.95)[2]),2)))
```
...
mypars = c('tqmgkgtot',
           ...
           'scale(CMAX)')
# 'scale(logpara0)')
plot(mod_everything,pars=mypars)
...
ind_tq = outcome_dat$tqmgkgtot>0
table(outcome_dat$AS_score[ind_tq], outcome_dat$country[ind_tq])

mod_cyp_t12=lmer(t_12_bw_adjusted ~ + as.factor(AS_score < 0.5) + (1|studysite),
...
```{r methb_ACT}
outcome_dat$ACT = factor(outcome_dat$ACT, levels = c('CQ','None','AL','DHA/PQP'))

outcome_dat$AMT = round(outcome_dat$weight*outcome_dat$tqmgkgtot)
boxplot(day7_mthb ~ ACT, data = outcome_dat[outcome_dat$AMT==300, ])

layout(mat = matrix(data = c(rep(1, 3*4),2:5),nrow = 4, byrow = F))
par(las=1, cex.axis=1.5, cex.lab=1.5, family='serif')
boxplot(log10(day7_mthb) ~ ACT, data = outcome_dat[outcome_dat$AMT==300, ],
...
for(act in c('CQ','AL','DHA/PQP','None')){
  hist(outcome_dat$weight[which(outcome_dat$AMT==300 & outcome_dat$ACT==act)],
       main=act, breaks=seq(35,140,by=5),ylab='', yaxt='n',xlab = 'weight (kg)')
}
mod_ACT = glm(log10(day7_mthb) ~ tqmgkgtot + as.numeric(ACT=='CQ'), 
              data = outcome_dat)
summary(mod_ACT)

table(outcome_dat$ACT[!is.na(outcome_dat$day7_mthb) & ! is.na(outcome_dat$tqmgkgtot)])
```
...
```{r phase1_AL_DHAPPQ}
mthb=read.csv(file = 'day_7_mthb.csv')
Lumefantrine_levels = read.csv(file = 'Day7_lumefantrine_levels.csv')
Piperaquine_levels = read.csv(file = 'Day7_piperaquine_levels.csv')

Lumefantrine_levels = merge(Lumefantrine_levels, mthb, by.x = 'ID', by.y = 'pid')
Piperaquine_levels = merge(Piperaquine_levels, mthb, by.x = 'ID', by.y = 'pid')

ylims = range(c(Lumefantrine_levels$day7_mthb, Piperaquine_levels$day7_mthb))

mthb = merge(mthb, pk_summaries, by.x ='pid', by.y='PID')
quants_TQ_alone =  quantile(mthb$day7_mthb[mthb$ACT=='None'], probs = c(0.1, .5, 0.9))

par(las=1, family='serif', cex.lab=1.3, cex.axis=1.3, mfrow=c(1,2))
plot(Lumefantrine_levels$Log10_Lumefantrine, log10(Lumefantrine_levels$day7_mthb),
     xlab = 'Day 7 lumefantrine concentration (ug/L)', xaxt='n',ylim=range(log10(ylims)),
     ylab = 'Day 7 methaemoglobin (%)', yaxt='n', panel.first=grid(),pch=16)
abline(h = log10(quants_TQ_alone), col='lightgrey', lwd=c(2,3,2),lty=2)
axis(1, at = log10(c(400, 1000, 2000, 4000)), labels = c(400, 1000, 2000, 4000))
axis(2, at = log10(c(0.7, 1, 2,4)), labels = c(.7, 1, 2,4))
mod0=lm(log10(day7_mthb)~Log10_Lumefantrine,data = Lumefantrine_levels)
abline(mod0,lwd=3)
summary(mod0)
abline(MASS::rlm(log10(day7_mthb)~Log10_Lumefantrine,data = Lumefantrine_levels),lty=2,lwd=3)

plot(Piperaquine_levels$Log10_Piperaquine, log10(Piperaquine_levels$day7_mthb),
     xlab = 'Day 7 piperaquine concentration (ug/L)', xaxt='n',ylim=range(log10(ylims)),
     ylab = 'Day 7 methaemoglobin (%)', yaxt='n', panel.first=grid(),pch=16)
axis(1, at = log10(c(20, 50,100, 140)), labels = c(20, 50,100, 140))
axis(2, at = log10(c(0.7, 1, 2,4)), labels = c(.7, 1, 2,4))
abline(h = log10(quants_TQ_alone), col='lightgrey', lwd=c(2,3,2),lty=2)

mod1=lm(log10(day7_mthb)~Log10_Piperaquine,data = Piperaquine_levels)
abline(mod1,lwd=3)
summary(mod1)
abline(MASS::rlm(log10(day7_mthb)~Log10_Piperaquine,data = Piperaquine_levels),lty=2,lwd=3)
```
